...
function Navbar() {
  ...
  const navLinkBase =
    'nav-link-animated rounded-md px-2 py-1.5 font-medium text-slate-700 transition-all duration-300 hover:bg-blue-50 hover:text-blue-700 whitespace-nowrap xl:px-2.5'
  const navLinkActive = 'nav-link-active bg-blue-100 text-blue-800 shadow-sm'
  const mobileNavLinkBase =
    'block px-4 py-2 font-medium text-slate-700 transition-all duration-300 hover:bg-blue-50 hover:text-blue-700 hover:translate-x-1'
  ...
  return (
    <header
      className={`sticky top-0 z-40 w-full border-b transition-all duration-500 ease-out ${
        isScrolled
          ? 'border-slate-200/80 bg-white/95 shadow-md shadow-slate-200/40 backdrop-blur-md'
          : 'border-transparent bg-white/80 backdrop-blur-sm'
      }`}
    >
      <div
        className={`mx-auto flex w-full max-w-7xl items-center justify-between gap-2 px-3 sm:px-4 transition-all duration-500 ${
          isScrolled ? 'py-2' : 'py-2.5 sm:py-3'
        }`}
      >
        <Link
          to="/"
          className="group shrink-0 rounded-md px-2 py-1 transition-transform duration-300"
        >
          <span className="logo-hover-shimmer bg-gradient-to-r from-blue-700 via-indigo-600 to-blue-800 bg-clip-text text-lg font-bold text-transparent transition-all group-hover:from-blue-600 group-hover:via-indigo-500 group-hover:to-blue-700">
            Bazarix
          </span>
        </Link>
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-end gap-1 text-xs xl:flex xl:gap-1.5 xl:text-sm">
          ...
          {isAuthenticated && role === 'customer' && (
            ...
            >
              Cart
              {cartCount > 0 && (
                <span className="absolute -right-1 -top-1 flex h-4 w-4 animate-pulse items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white">
                  {cartCount}
                </span>
              )}
            </NavLink>
            ...
            <>
              <div ref={accountMenuRef} className="relative">
                ...
                {isAccountOpen ? (
                  <div
                    role="menu"
                    className="animate-scale-up-fade origin-top-right absolute right-0 top-full z-50 mt-1 min-w-[11rem] rounded-lg border border-slate-200 bg-white py-1 shadow-lg"
                  >
              ...
              <span className="max-w-[7rem] truncate rounded-full bg-slate-100 px-2.5 py-1 text-slate-600 transition-all duration-300 hover:bg-blue-50 hover:text-blue-700 xl:max-w-[9rem]">
                {user?.name || 'User'}
              </span>

              <button
                type="button"
                className="shrink-0 rounded-md bg-red-500 px-2.5 py-1.5 text-xs font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-red-600 hover:shadow-md xl:px-3 xl:text-sm"
      ...
      {isMenuOpen ? (
        <div
          ref={mobileMenuRef}
          className="animate-fade-in-down border-b border-slate-200 bg-white/95 backdrop-blur-md xl:hidden"
        >
      ...
      {showToast && (
        <div className="animate-fade-in-up fixed bottom-6 right-4 z-50 max-w-[calc(100vw-2rem)] rounded-lg bg-green-600 px-4 py-3 text-sm font-semibold text-white shadow-lg sm:right-6 sm:px-5">
