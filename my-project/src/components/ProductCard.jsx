...
function ProductCard({ product, onAddToCart }) {
  ...
  return (
    <article className="relative flex flex-col justify-between h-[440px] w-full rounded-lg border border-slate-100 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      <button
        type="button"
      ...
      {/* Top scrollable/structured area */}
      <div className="flex flex-col flex-grow overflow-hidden">
        <div className="h-48 w-full overflow-hidden rounded-md bg-slate-50">
          <img
            src={product.image}
            alt={product.name}
        ...
        <h3 className="mt-3 text-base font-semibold text-slate-900 line-clamp-2 h-12 overflow-hidden leading-6" title={product.name}>
          {product.name}
        </h3>
        <p className="mt-1 text-xs text-slate-500 truncate">{product.category}</p>
        <p className="mt-1 text-xs text-slate-500 truncate">{product.sellerName || product.seller}</p>
      ...
      <button
        className="mt-4 w-full rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-blue-700 hover:shadow-md"
